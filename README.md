...
- 自定义端口（默认 443）
- 批量操作支持
- 支持 DNS 黑名单和节点黑名单两套独立规则
- DNS 黑名单中的 CFIP 不参与 CF/DNS 同步，节点黑名单中的 CFIP 默认不参与订阅生成
...
**订阅特性：**
- 自动组合所有启用且符合订阅配置的 CFIP 和 ProxyIP/Outbound
...
1. **指定单个ProxyIP**：
   ...
   ```
   生成ID为1的ProxyIP × 所有启用且符合订阅配置的CFIP的订阅

2. **指定多个ProxyIP**：
   ```
   https://你的Pages域名/sub/你的UUID?proxyip=1,3,5
   ```
   生成ID为1、3、5的ProxyIP × 所有启用且符合订阅配置的CFIP的订阅

3. **指定全局出站**：
   ```
   https://你的Pages域名/sub/你的UUID?outbound=2,4
   ```
   生成ID为2、4的全局出站 × 所有启用且符合订阅配置的CFIP的订阅

4. **同时指定ProxyIP和全局出站**：
   ```
   https://你的Pages域名/sub/你的UUID?proxyip=1&outbound=2
   ```
   生成ID为1的ProxyIP和ID为2的全局出站 × 所有启用且符合订阅配置的CFIP的订阅

5. **指定CFIP**：
   ```
   https://你的Pages域名/sub/你的UUID?cfip=1,2,3
   ```
   生成所有启用的ProxyIP/全局出站 × ID为1、2、3且符合订阅配置的CFIP的订阅
...
**重要说明：**
- 指定ID时，**不管该ID是否启用状态**，都会被包含在订阅中；节点黑名单 CFIP 是否包含由订阅配置决定
- 未指定参数时，使用所有**启用状态**的项目
- 指定`proxyip`或`outbound`参数时，只会使用指定的ID，不会包含其他启用的项目
- 指定`cfip`参数时，只会使用指定且符合订阅配置的 CFIP ID
- 可通过 `include_blacklisted_cfip=1` 临时让公开订阅包含节点黑名单 CFIP，例如：`/sub/你的UUID?include_blacklisted_cfip=1`
- 可以灵活组合参数，生成不同的订阅链接
- 节点数量 = (指定的ProxyIP数量 + 指定的Outbound数量) × 指定且符合订阅配置的CFIP数量
- DNS 黑名单始终影响 CF 同步，不会从管理列表删除记录；节点黑名单是否参与订阅输出由对应订阅配置决定
...
- 勾选多个项目后，可批量启用/禁用/删除
- 支持批量测速和出站检测
- CFIP 支持按失败次数阈值批量删除，例如删除失败次数大于等于指定值的记录
